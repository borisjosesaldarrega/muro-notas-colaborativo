...
function openScreen(kind) {
  ...
  const titles = { profile: ['Tu cuenta', 'Mi perfil'], settings: ['Preferencias', 'Configuración'], admin: ['Superadministración', 'Administración'] }; $('#screenEyebrow').textContent = titles[kind][0]; $('#screenTitle').textContent = titles[kind][1];
...
function renderAdmin() {
  ...
  function drawUsers(query = '') { table.replaceChildren(); state.admin.users.filter((user) => `${user.name} ${user.email}`.toLowerCase().includes(query.toLowerCase())).forEach((user) => { const row = create('article', 'data-row'); const info = create('div'); info.append(create('strong', '', user.name), create('small', '', user.email)); const select = create('select'); ['usuario', 'superadmin'].forEach((value) => { const option = create('option', '', value); option.value = value; option.selected = user.role === value; select.append(option); }); select.addEventListener('change', async () => { if (!(await confirmAction(`¿Cambiar el rol de ${user.name} a ${select.value}?`))) { select.value = user.role; return; } const result = await emitAck('admin:user:role', { userId: user.id, role: select.value }); if (!result?.ok) { select.value = user.role; return toast(result?.error, true); } await loadAdmin(); }); row.append(info, create('span', 'date-cell', formatDate(user.createdAt)), select); table.append(row); }); } search.addEventListener('input', () => drawUsers(search.value)); drawUsers();
