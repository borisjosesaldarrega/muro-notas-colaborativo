...
class SupabaseStore {
  ...
  async loadState() {
    const [profilesResult, settingsResult, wallsResult, membersResult, notesResult] = await Promise.all([
      this.admin.from('profiles').select('id,name,email,role,avatar,created_at'),
      this.admin.from('user_settings').select('user_id,theme,confirm_delete,compact_notes,notifications'),
      this.admin.from('walls').select('id,name,description,owner_id,created_at,updated_at'),
      this.admin.from('wall_members').select('wall_id,user_id,role'),
      this.admin.from('notes').select('id,wall_id,text,color,x,y,author_id,author_name,created_at,updated_at')
    ]);
  ...
  async getUser(id) {
    const [profileResult, settingsResult] = await Promise.all([
      this.admin.from('profiles').select('id,name,email,role,avatar,created_at').eq('id', id).single(),
      this.admin.from('user_settings').select('theme,confirm_delete,compact_notes,notifications').eq('user_id', id).single()
    ]);
  ...
  async saveProfile(user, accessToken) {
    const { error } = await this.userClient(accessToken).from('profiles').update({ name: user.name, avatar: user.avatar, updated_at: new Date().toISOString() }).eq('id', user.id);
    throwIf(error, 'profile_update');
  }

  async saveSettings(user, accessToken) {
    const { error } = await this.userClient(accessToken).from('user_settings').update({
  ...
  async saveWall(wall, accessToken) {
    const { error } = await this.userClient(accessToken).from('walls').upsert({
      id: wall.id,
      name: wall.name,
  ...
  async deleteWall(id, accessToken) {
    const { error } = await this.userClient(accessToken).from('walls').delete().eq('id', id);
    throwIf(error, 'wall_delete');
  }

  async saveMember(wallId, userId, role, accessToken) {
    const { error } = await this.userClient(accessToken).from('wall_members').upsert({ wall_id: wallId, user_id: userId, role });
    throwIf(error, 'member_save');
  }

  async deleteMember(wallId, userId, accessToken) {
    const { error } = await this.userClient(accessToken).from('wall_members').delete().eq('wall_id', wallId).eq('user_id', userId);
    throwIf(error, 'member_delete');
  }

  async saveNote(wallId, note, accessToken) {
    const { error } = await this.userClient(accessToken).from('notes').upsert({
      id: note.id,
      wall_id: wallId,
      text: note.texto,
      color: note.color,
  ...
  async deleteNote(id, accessToken) {
    const { error } = await this.userClient(accessToken).from('notes').delete().eq('id', id);
    throwIf(error, 'note_delete');
  }

  async clearNotes(wallId) {
    const { error } = await this.admin.from('notes').delete().eq('wall_id', wallId);
    throwIf(error, 'notes_clear');
  }

  async saveRole(user) {
    const { error } = await this.admin.from('profiles').update({ role: user.role, updated_at: new Date().toISOString() }).eq('id', user.id);
