...
class SupabaseStore {
  ...
  async createWall(wall, accessToken) {
    const { error } = await this.userClient(accessToken).from('muro_walls').insert(this.wallRow(wall));
  ...
  async updateWall(wall, accessToken) {
    ...
    delete row.id;
    delete row.owner_id;
    const { error } = await this.userClient(accessToken).from('muro_walls').update(row).eq('id', wall.id);
  ...
  async createMember(wallId, userId, role, accessToken) {
    const { error } = await this.userClient(accessToken).from('muro_wall_members').insert({ wall_id: wallId, user_id: userId, role });
    throwIf(error, 'member_save');
  }

  async updateMember(wallId, userId, role, accessToken) {
    const { error } = await this.userClient(accessToken).from('muro_wall_members').update({ role }).eq('wall_id', wallId).eq('user_id', userId);
  ...
  async createNote(wallId, note, accessToken) {
    const { error } = await this.userClient(accessToken).from('muro_notes').insert(this.noteRow(wallId, note));
  ...
  async updateNote(wallId, note, accessToken) {
    ...
    const { error } = await this.userClient(accessToken).from('muro_notes').update(row).eq('id', note.id).eq('wall_id', wallId);
